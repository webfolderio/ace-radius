 ...
 */
#if WORDS_BIGENDIAN
    void byteReverse(unsigned char *buf, uint32_t longs)
    {
        uint32_t t;
        do {
        t = ((unsigned long) buf[3] << 8 | buf[2]) << 16 |
            ((unsigned long) buf[1] << 8 | buf[0]);

        memcpy(buf, (unsigned char *)&t, 4);
...
/*
 * Complete MD5 algorithm in a single function
 */
void MD5Calc(unsigned char *output,unsigned char *input, uint32_t len)
{
    MD5Context context;
    MD5Init(&context);
